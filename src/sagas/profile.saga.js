...
export function* watchChangePasswordSucceeded() {
  yield* takeLatest(
    types.CHANGE_PASSWORD__SUCCEEDED,
    doChangePasswordSucceeded
  );
}
...
export function* doChangePasswordFailed(action) {
  ...
  const [currentPassword, newPassword, newPasswordRepeated] = [
    ...
    yield call(
      formErrorHelper,
      errorData,
      "children.plainPassword.children.first.errors"
    ),
    yield call(
      formErrorHelper,
      errorData,
      "children.plainPassword.children.second.errors"
    )
  ];

  yield put(
    stopSubmit("change-password", {
      currentPassword,
      newPassword,
      newPasswordRepeated
    })
